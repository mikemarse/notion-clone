...
const SeventhSection = () => {
	return (<div className="mt-32 flex flex-col justify-center items-center">
		...
		<div className="grid grid-cols-12 grid-row-3 md:row-span-2 gap-4 xl:gap-6 mt-8 px-8 md:px-16 xl:px-0 xl:w-[990px] 2xl:w-[70%] mx-auto md:w-full">
			<div className="bg-[#f6f5f4] flex flex-col col-span-6 row-span-3 rounded-xl shadow-md cursor-pointer hover:bg-[#EAE7E7]">
			...
			{tabs.map((tab, index) => (
				<div className="bg-[#f6f5f4] cursor-pointer hover:bg-[#EAE7E7] py-6 px-8 col-span-3 rounded-xl" key={index}>
					<div className="flex flex-col justify-center">
						<div>{tab.icon}</div>
						<div className="text-lg font-medium mt-2">{tab.title}</div>
						<div className="flex items-center text-center text-sky-600 hover:underline cursor-pointer mt-8">Get template <PiArrowRight className="ml-1" /> </div>
					</div>
				</div>
			))}
		</div>
